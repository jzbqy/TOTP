...
function isStandalone2FAPage() {
  ...
  // 新增规则：如果URL包含action=security，认为是同一页面，不自动填充
  ...
  for (const element of allElements) {
    ...
    if (text.includes('两步验证') || text.includes('两步認證') || 
        text.includes('二步验证') || text.includes('二步認證') ||
        text.includes('双重验证') || text.includes('雙重驗證') ||
        text.includes('two-factor') || text.includes('2fa') ||
        text.includes('multi-factor') || text.includes('totp')) {
  ...
  for (const element of allElements) {
    const text = element.textContent || '';
    if (text.includes('安全设定') || text.includes('安全設定') ||
        text.includes('安全设置') || text.includes('安全設置') ||
        text.includes('security settings') || text.includes('account security')) {
